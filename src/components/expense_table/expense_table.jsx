...
const ExpenseTable = () => {
	...
	<div className="table-filter">
        <select className='filter-dropdown' onChange={onExpenseTypeChange}>
          {expenseTypes.map(n => (
            <option className='filter-dropdown-item' value={n}>{n}</option>
          ))}
        </select>
	</div>
	...
	<Modal
      ...
      >
		...
		<h4 style={{fontWeight:"200"}}>Are you sure you want to delete this expense? This action is IRREVERSIBLE</h4>
		<div style={{display:"flex",flexDirection:"row",justifyContent:"flex-end",marginTop:"20px"}}>
			<button style={buttonStyle("#4aa088")} onClick={onDeleteExpenseClick}>Cancel</button>
			<button style={buttonStyle("linear-gradient(to right, #ff416c, #ff4b2b)")} >Delete</button>
		</div>
	  </Modal>
					...
					Array.from(Array(20).keys()).map((e)=>{
						...
						<td>
							<div className="table-entry-actions">
								...
								<i className="bx bxs-trash" id="table-entry-icon" onClick={onDeleteExpenseClick} style={{color:"#de2146"}}/>
							</div>
						</td>
						</tr>)
					})
